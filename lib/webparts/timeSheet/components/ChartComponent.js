...
var ChartComponent = function (_a) {
    ...
    useEffect(function () {
        var loadChartJS = function () { return __awaiter(void 0, void 0, void 0, function () {
...
function getChartConfig(data, type) {
    ...
    var chartData = __assign({}, data);
    // Configure datasets based on chart type
    if (chartData.datasets) {
        chartData.datasets = chartData.datasets.map(function (dataset, index) {
            var baseDataset = __assign({}, dataset);
            if (type === 'line') {
                return __assign(__assign({}, baseDataset), { backgroundColor: dataset.backgroundColor || 'rgba(59, 130, 246, 0.1)', borderColor: dataset.borderColor || '#3b82f6', borderWidth: 3, fill: true, tension: 0.4, pointBackgroundColor: '#3b82f6', pointBorderColor: '#ffffff', pointBorderWidth: 2, pointRadius: 5, pointHoverRadius: 7 });
            }
            if (type === 'bar') {
                return __assign(__assign({}, baseDataset), { backgroundColor: dataset.backgroundColor || '#3b82f6', borderColor: 'transparent', borderRadius: 6, borderSkipped: false });
            }
            if (type === 'pie' || type === 'doughnut') {
                ...
                return __assign(__assign({}, baseDataset), { backgroundColor: colors.slice(0, dataset.data.length), borderWidth: 2, borderColor: '#ffffff' });
            }
            return baseDataset;
        });
    }
    return {
        type: type,
        data: chartData,
        options: __assign(__assign({}, commonOptions), (type === 'doughnut' && {
            cutout: '60%'
        }))
    };
}
...
function drawFallbackChart(ctx, data, type, width, height) {
    ...
    if (type === 'line' && data.labels) {
        ...
        var minValue_1 = Math.min.apply(Math, __spreadArray(__spreadArray([], values, false), [0], false));
        ...
        // Simple bar chart
        var maxValue_1 = Math.max.apply(Math, __spreadArray(__spreadArray([], values, false), [1], false));
        var padding_2 = 30;
