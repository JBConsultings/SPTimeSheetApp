...
function generateRecentActivity(entries) {
    ...
    return recentEntries.map(function (entry) {
        ...
        if (entry.status === 'Submitted' && entry.submittedOn) {
            type = 'submitted';
            description = "Submitted timesheet for ".concat(formatDateShort(entry.entryDate), " - ").concat(entry.projectName);
        }
        else if (entry.status === 'Approved') {
            type = 'approved';
            description = "Timesheet approved for ".concat(formatDateShort(entry.entryDate), " - ").concat(entry.projectName);
        }
        else if (entry.status === 'Rejected') {
            type = 'rejected';
            description = "Timesheet rejected for ".concat(formatDateShort(entry.entryDate), " - ").concat(entry.projectName);
        }
        else {
            type = 'submitted';
            description = "Updated timesheet for ".concat(formatDateShort(entry.entryDate), " - ").concat(entry.projectName);
        }
        return {
...
export function getAnalyticsData(employeeEmail) {
